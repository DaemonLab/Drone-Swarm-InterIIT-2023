...
class pluto:
    ...
    def __init__(self, DroneIP="192.168.4.1", DronePort="23"):
        ...
        self.roll_byts = bytearray([220,5])
        self.pitch_byts = bytearray([220,5])
        self.throttle_byts = bytearray([220,5])
        self.yaw_byts = bytearray([220,5])

        self.aux1_byts = bytearray([176,4])
        self.aux1_byts = bytearray([232,3])
        self.aux1_byts = bytearray([220,5])
        self.aux1_byts = bytearray([176,4])
    ...
    def create_datapacket(self,dir, ls_data , cmdtype ):

        final_data = []

        final_data = self.header + self.dir + 2*len(ls_data) + cmdtype  # not complete here
    ...
    def req(self,type):
        """Give type of data req"""

        if type == "ALT":
            arro = bytearray([36,77,62,6,109,109])
            self.speedz(0)
            i = 0 
            while(i<20):
                val = self.tn.read_lazy()
        ...
        if type == "IMU":
            arro = bytearray([36,77,62,18,102,0])
            val=self.changeCRC(arro)
            arro[5]=val
            self.speedz(0)
            i = 0 
            while(i<5):
                val = self.tn.write(arro)
                print(f"returned {val}")
                time.sleep(0.5)
                self.speedz(0)
                i += 1
