...
class MissionPlanner():
    ...
    def __init__(self,conn) :
        ...
        self.Drone1=pluto(DroneIP='192.168.4.1')
        self.Drone1.connect()
        self.Drone1.disarm()
    ...
    def recv_pose(self):
        ...
        while ( self.Task_not_done ):
            if (self.conn.poll()):                
                ...
                self.POSEDICT[self.Drone1ID] = []
                self.POSEDICT[self.Drone2ID] = []


            print(f"\nRec : Pose Dict: {self.POSEDICT}")
    ...
    def move( self, temp_targets , DroneID , Err , ErrI):
        ...
        pose = self.POSEDICT[DroneID] #pose is array
        ...
        while temp_target_idx != len(temp_targets)  : #loop till idx just greater than last ele
            ...
            pose = self.POSEDICT[DroneID] #pose is an array

            

            try:
                    
                if len(pose)==0 : # pose is None
                    
                    now_time = time.time()
                    time_not_detected = now_time - start 

                    roll_command, pitch_command, throttle_command, yaw_command = 0, 0, 70, 0

                    if time_not_detected> 8 : 
                        print(f"\nAruco not detected ,landing : {DroneID}\n")
                        Droneobj.land()
                        return 'not det'
                        
                    
                elif len(pose) != 0 : #Pose not None
                    ...
                    if np.sqrt((xTarget-pose[0])**2+(yTarget-pose[1])**2)<25:
                        ...
                        if (temp_target_idx < (len(temp_targets)-1) and tReachCount>= 5 ): 
                            print(f"\nOn the way Target Reached. : {DroneID} , Target : {temp_target_idx}\n")
                            temp_target_idx +=1
                            tReachCount=0
                ...
                print(f"\nMove : {DroneID}--- Pose:--{pose},  ")
                # print(f"\nbefore sending-----------------{self.POSEDICT}")
    ...
    def dronePlan(self,DroneID):
        ...
        while not Rectangle_done :
            ...
            move_state = self.MOVEDICT[DroneID]
            print(f"\n***** State :{DroneID} , State T/F : { move_state }")        

            if move_state :  #true at very first for drone1 and False for drone2
                resp = self.move(self.target_array[IDX], DroneID=DroneID,Err=Err,ErrI=ErrI  )
                ...
                print(f"\n-------------\n...Are..Reached corner 1 of {DroneID}-----\n")
                ...
                if( IDX == (self.START_OUTER_IDX[DroneID]-1) %4 ):
                    Rectangle_done  = True
                    DroneObj.land()
                    if (DroneID == self.Drone1ID) : 
                        self.Rectangle1_done = True
                    else:
                        self.Rectangle2_done = True
