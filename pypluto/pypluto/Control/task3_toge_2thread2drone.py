...
class CameraTask3():
    ...
    def __init__(self):
        ...
        self.KPx, self.KPy, self.KPz, self.KPyaw = 0.8, 0.4, 380 , 50
        self.KIx, self.KIy, self.KIz, self.KIyaw = 0.02, 0.01, 0, 0
        self.KDx, self.KDy, self.KDz, self.KDyaw = 18, 25, 10, 0

        self.CAMERA_HEIGHT = 1.9 

        self.Aruco_ref_dist = 2.2 # may be same as camera ht if we use it as a reference
        self.Aruco_ht_pixels_grnd =15 #ht in pixels when aruco on ground( reference dist)
        self.Aruco_width_pixels_grnd = 15 #wdth in pixels when aruco on ground( reference dist)
        self.Aruco_len_pixels_grnd = 15
...
class MissionPlanner(CameraTask3):
    ...
    def pid(self,pose, target, Err, ErrI):
        ...
        # compute commands
        xCommand = self.KPx*x_error + self.KIx*x_errorI + self.self.KDx*x_errorD
        yCommand = self.KPy*y_error + self.KIy*y_errorI + self.KDy*y_errorD
        zCommand = self.KPz*z_error + self.KIz*z_errorI + self.KDz*z_errorD

        yaw_command    = int( self.KPyaw*yaw_error + self.KIyaw*yaw_errorI + self.KDyaw*yaw_errorD)
        ...
        while temp_target_idx != len(temp_targets)  : #loop till idx just greater than last ele
            ...
            try:
                ...
                if not pose : # pose is None
                    ...
                    roll_command, pitch_command, throttle_command, yaw_command, Err, ErrI = self.pid(pose, xTarget, yTarget, heightTarget, Err, ErrI)
    ...
    def get_pose(self, corners, ids, image, desiredVec, display=True):
        ...
        if len(corners) > 0:
            ...
            for (markerCorner, markerID) in zip(corners, ids):
                ...
                rvec, tvec, markerPoints = cv2.aruco.estimatePoseSingleMarkers(np.array(corners), 0.02, cameraMatrix=self.MATRIX_COEFFICIENTS, distCoeffs=self.DISTORTION_COEFFICIENTS)
